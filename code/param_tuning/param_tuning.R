...
date = as.integer(args[2]) # date-id
which2tune = as.integer(args[3]) # indicator of which param to tune, 2 for lambda_2, 3 for lambda_3

# load the data
path = "./"
date = datelist[date]
TEC = readMat(paste(path,"TEC_",date,"_mask.mat",sep=""))
TEC_mask = TEC$tec.mask # tec data to fit
TEC = TEC$tec # tec data with observations on the masked region
SH = readMat(paste(path,"SH_",date,"_mask.mat",sep=""))
...
output_path = "./result/"
fname = paste(output_path, date, "_", which2tune, "_", p, ".csv",sep="")
write.csv(df, fname)
